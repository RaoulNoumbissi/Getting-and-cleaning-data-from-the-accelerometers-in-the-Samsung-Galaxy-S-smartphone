...
#here we merge the subject training set from the subject_train.txt file to our new training set.
# the resulting dataframe is called new_smartphone_dataset.
...
new_smartphone_dataset<- cbind(subject_train,new_X_train,label)


#Finally the aggregate function allows us to find the mean of all variables for each subject and for each activity
by_Subject_by_Activity<-aggregate(new_smartphone_dataset[, 2:67], list(new_smartphone_dataset$subject,new_smartphone_dataset$label), mean)
